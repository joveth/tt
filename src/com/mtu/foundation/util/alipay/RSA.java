...
public class RSA {
	...
	public static String decrypt(String content, String private_key,
		...
		while ((bufl = ins.read(buf)) != -1) {
			...
			byte[] temp = cipher.doFinal(block, 0, block.length);
			System.out.println(new String(temp, input_charset));
			writer.write(temp);
		}
		byte[] temp = writer.toByteArray();
		return new String(temp, input_charset);
	}
	...
	public static PrivateKey getPrivateKey(String key) throws Exception {
		...
		keyBytes = com.alipay.sdk.encrypt.Base64.a(key);
	...
	public static String getRequestToken(String text) throws Exception {
		...
		if (paraText.get("res_data") != null) {
			...
			request_token = RSA.decrypt(res_data, AliPayUtil.RSA_PRIVATE,
					AliPayUtil.CHARSET);
			// token从res_data中解析出来（也就是说res_data中已经包含token的内容）
			Log.d("request_token", request_token);
			request_token = request_token
					.substring(request_token.indexOf("<?"));
...
public static String decrypt(String content, String private_key) throws Exception {
		...
		PrivateKey prikey =  getPrivateKey(Base64Sec.decode(private_key));
			
		Cipher cipher = Cipher.getInstance("RSA");
		cipher.init(Cipher.DECRYPT_MODE, prikey);
		InputStream ins = new ByteArrayInputStream(Base64Sec.decode(content));
		ByteArrayOutputStream writer = new ByteArrayOutputStream();
		byte[] buf = new byte[128];
		int bufl;
