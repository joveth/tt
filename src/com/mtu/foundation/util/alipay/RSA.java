...
public class RSA {
    ...
    public static String getRequestToken(String text) throws Exception {
        ...
        if (paraText.get("res_data") != null) {
            ...
            request_token = RSA.decrypt(res_data, AliPayUtil.RSA_PRIVATE, AliPayUtil.CHARSET);
            //token从res_data中解析出来（也就是说res_data中已经包含token的内容）
            Log.d("request_token", request_token);
           request_token = request_token.substring(request_token.indexOf("<?"));
            int startIndex = request_token.indexOf("<request_token>");
            if (startIndex > -1) {
                int temp = "<request_token>".length();
                String start = request_token.substring(temp+startIndex, temp+startIndex+6);
                Log.d("start", start);
