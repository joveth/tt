...
public class AliPayWapActivity extends BaseActivity {
    ...
    private void doCallRemote() {
        ...
        if (progressDialog == null) {
            ...
            progressDialog.setOnCancelListener(new DialogInterface.OnCancelListener() {
                ...
                public void onCancel(DialogInterface dialogInterface) {
                    cancelFlag = true;
                    Toast.makeText(AliPayWapActivity.this, "交易被取消!", Toast.LENGTH_SHORT).show();
                    finish();
                }
            });
        }
        ...
        String req = AliPayUtil.getAliPayWAPOrderBodyInfor("test", price, "1282889689801", SUCCESS_URL, NOTIFY_URL, userid);
        Log.d("req_data",req);
        ...
        networkHandler.post(AliPayUtil.ALIPAY_WAP_URL,paramspost, 15, new Callback<TransResp>() {
            @Override
            public void callback(TransResp transResp) {
                ...
                if (transResp.getRetcode() == HttpStatus.SC_OK) {
                    if (CommonUtil.isEmpty(transResp.getRetjson())) {
                        progressDialog.dismiss();
                        Toast.makeText(AliPayWapActivity.this, "请求失败!", Toast.LENGTH_SHORT).show();
                        finish();
                        return;
                    }
                    String dec = "";
                    try {
                        dec = URLDecoder.decode(transResp.getRetjson(), "utf-8");
                    } catch (UnsupportedEncodingException e) {
                        e.printStackTrace();
                        progressDialog.dismiss();
                        Toast.makeText(AliPayWapActivity.this, "请求内容解析失败，请稍后再试!", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    String ret;
                    try {
                    ...
                    progressDialog.dismiss();
                    Toast.makeText(AliPayWapActivity.this, "请求未响应!", Toast.LENGTH_SHORT).show();
                    return;
                }
            }
        });
    ...
    private void wapPay(String token) {
        ...
        sParaTemp.put("sign", mysign);
        List<NameValuePair> paramspost = generatNameValuePair(sParaTemp);
        String url = AliPayUtil.ALIPAY_WAP_URL + "?" + URLEncodedUtils.format(paramspost, "UTF-8");
        Log.d("url", url);
        webView.loadUrl(url);
    }
