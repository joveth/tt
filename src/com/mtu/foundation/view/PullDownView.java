...
/**
 * @version 1.0
 * @function 下拉刷新控件 真正实现下拉刷新的是这个控件， ScrollOverListView只是提供触摸的事件等
 */
...
public class PullDownView extends LinearLayout implements ScrollOverListView.OnScrollOverListener {
    ...
    public void enableAutoFetchMore(boolean enable, int index) {
        if (enable) {
            ...
            mFooterTextView.setText("点击加载更多");// 不自动加载
    ...
    public void setPullHeadBackgroundColor(int resid) {
        if (mListView != null) {
            mListView.setPullHeadBackgroundColor(resid);
        }
    }
    ...
    public void setInnerScrollListenr(ScrollOverListView.InnerScrollListenr scrollListenr) {
        mListView.setInnerScrollListenr(scrollListenr);
    }
    @Override
